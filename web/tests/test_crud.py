...
class TestCrudActions(object):
    ...
    def test_list(self, app, model):
        for i in range(1, 12):
            model.create({'title': u'User {0}'.format(i)})
        ...
        assert 'User 1' in body, "Should show users"
        # Ordering is lexical, so 9 will be on page two and 10 and 11 will
        # be after 1.
        assert 'User 9' not in body, "Should not show more than 10 users"
    ...
    def test_add(self, app):
        data = {
            'name': 'Test User',
            'phonenumber': '5551212',
            'email': 'foo@foocorp.zz',
        }

        with app.test_client() as c:
            rv = c.post('/users/add', data=data, follow_redirects=True)

        assert rv.status == '200 OK'
        body = rv.data.decode('utf-8')
        assert 'Test User' in body
        assert '5551212' in body
        assert 'foo@foocorp.zz' in body
