...
def model(monkeypatch, app):
    ...
    """
    model = gainful.get_model()

    # Ensure no users exist before running. This typically helps if tests
    # somehow left the database in a bad state.
    delete_all_users(model)

    yield model

    # Delete all users that we created during tests.
    delete_all_users(model)
...
def delete_all_users(model):
    while True:
        users, _ = model.list(limit=50)
        if not users:
            break
        for user in users:
